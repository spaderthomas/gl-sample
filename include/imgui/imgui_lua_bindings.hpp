...
const char * RunString(const char* szLua) {
  if (!get_lua().raw_state) {
    fprintf(stderr, "You didn't assign the global lState, either assign that or refactor LoadImguiBindings and RunString\n");
  }

  int iStatus = luaL_loadstring(get_lua().raw_state, szLua);
  if(iStatus) {
    return lua_tostring(get_lua().raw_state, -1);
    //fprintf(stderr, "Lua syntax error: %s\n", lua_tostring(get_lua().raw_state, -1));
    //return;
  }
#ifdef ENABLE_IM_LUA_END_STACK
  endStack.clear();
#endif
  iStatus = lua_pcall( get_lua().raw_state, 0, 0, 0 );
...
#define IMGUI_KEY_ARG(name) \
  const ImGuiKey name = (ImGuiKey)((int)luaL_checknumber(L, arg++));
...
static void PushImguiEnums(lua_State* lState, const char* tableName) {
  lua_pushstring(get_lua().raw_state, tableName);
  lua_newtable(get_lua().raw_state);

#undef START_ENUM
#undef MAKE_ENUM
#undef END_ENUM
#define START_ENUM(name) \
  lua_pushstring(get_lua().raw_state, #name); \
  lua_newtable(get_lua().raw_state); \
  { \
    int i = 1;
#define MAKE_ENUM(c_name,lua_name) \
  lua_pushstring(get_lua().raw_state, #lua_name); \
  lua_pushnumber(get_lua().raw_state, c_name); \
  lua_rawset(get_lua().raw_state, -3);
#define END_ENUM(name) \
  } \
  lua_rawset(get_lua().raw_state, -3);
// These defines are just redefining everything to nothing so
